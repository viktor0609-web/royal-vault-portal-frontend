...
export const PeoplePanel: React.FC<PeoplePanelProps> = ({ onClose }) => {
  ...
  const userVideoTrack = participants.find(p => p.local && !p.permissions.canAdmin)?.videoTrack;
    ...
    <div className="w-80 bg-gray-900 text-white p-4 flex flex-col">
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="grid grid-cols-1 gap-2">
          ...
          {/* User video - only show if not in main video and video is enabled */}
          {userVideoTrack && userVideoTrack !== mainVideoTrack && participants.find(p => p.local && !p.permissions.canAdmin)?.videoTrack && (
            <div className="relative bg-gray-800 rounded-lg overflow-hidden h-20">
              <VideoPlayer track={userVideoTrack} type="camera" thumbnail={true} />
              <div className="absolute bottom-1 left-1 text-white bg-black bg-opacity-50 px-1 py-0.5 rounded text-xs">
                User
              </div>
            </div>
