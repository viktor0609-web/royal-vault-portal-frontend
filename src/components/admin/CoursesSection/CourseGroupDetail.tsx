...
import type { CourseGroup, Course, CourseResource } from "@/types";
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export function CourseGroupDetail() {
    ...
    const fetchCourseGroup = async () => {
        ...
        try {
            ...
            const courseGroupData = response.data;
            setCourseGroup(courseGroupData as CourseGroup);
            setCourses((courseGroupData.courses || []) as Course[]);
        } catch (err) {
            const error = err as AxiosError<{ message?: string }>;
            const errorMessage = error.response?.data?.message || 'Failed to fetch course group';
            setError(errorMessage);
            toast({
                title: "Error",
                description: errorMessage,
                                ...
                                courses.map((course) => (
                                    <TableRow key={course._id} onClick={() => handleViewCourse(course._id)} className="cursor-pointer">
                                        ...
                                        <TableCell className="hidden xl:table-cell">
                                            {typeof course.createdBy === 'object' ? course.createdBy?.name : 'N/A'}
                                        </TableCell>
