...
export const dealService = {
  ...
  getDealById: (dealId: string, fields: FieldSelection = "full") =>
    api.get<{ message: string; deal: Deal }>(`${API_ENDPOINTS.DEALS.BASE}/${dealId}?fields=${fields}`),

  // Create new deal
  createDeal: (dealData: Partial<Deal>) =>
    api.post<{ message: string; deal: Deal }>(API_ENDPOINTS.DEALS.BASE, dealData),

  // Update deal
  updateDeal: (dealId: string, dealData: Partial<Deal>) =>
    api.put<{ message: string; deal: Deal }>(`${API_ENDPOINTS.DEALS.BASE}/${dealId}`, dealData),
