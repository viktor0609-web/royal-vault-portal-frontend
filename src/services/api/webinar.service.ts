...
export const webinarService = {
  ...
  getAllWebinars: (
    ...
    return api.get<{ message: string; webinars: Webinar[]; count: number }>(`${API_ENDPOINTS.WEBINARS.ADMIN}?${params.toString()}`);
  },
  ...
  getPublicWebinars: (
    ...
    if (filters?.status) params.append("status", filters.status);
    if (filters?.streamType) params.append("streamType", filters.streamType);
    return api.get<{ message: string; webinars: Webinar[]; count: number }>(`${API_ENDPOINTS.WEBINARS.PUBLIC}?${params.toString()}`);
  },
